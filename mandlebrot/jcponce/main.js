// SOURCE: https://github.com/jcponce/complex
...
class MandelbrotSet {
  ...
  update() {
    ...
    if (up === -1 || keyIsDown(KC_UP)) this.pos.y -= moveSpeed
    if (down === -2 || keyIsDown(KC_DOWN)) this.pos.y += moveSpeed
    if (left === -3 || keyIsDown(KC_LEFT)) this.pos.x -= moveSpeed
    if (right === -4 || keyIsDown(KC_RIGHT)) this.pos.x += moveSpeed
  ...
  drawOrbit() {
    ...
    while (
      count < this.maxIter &&
      dist(virtualPos.x, virtualPos.y, 0, 0) < 10
    ) {
  ...
  plot() {
    ...
    var cX =
      this.pos.x +
      map(mouseX, ctlsBack, width, -this.size.x / 2, this.size.x / 2) //this is for Mandelbrot
    var cY =
      this.pos.y + map(mouseY, height, 0, -this.size.y / 2, this.size.y / 2) //this is for Mandelbrot
    ...
    for (var x = ctlsBack; x < width; x++) {
      for (var y = 0; y < height; y++) {
        ...
        var z = new p5.Vector(
          this.pos.x +
            map(x, ctlsBack, width, -this.size.x / 2, this.size.x / 2),
        ...
        while (iter < this.maxIter) {
          ...
          z.x = sqZ.x + c.x
          z.y = sqZ.y + c.y
          if (abs(z.x + z.y) > 16) break
          iter++
        }
        setPixelHSV(
          x,
          y,
          map(iter, 0, this.maxIter, 0, 1),
          1,
          iter !== this.maxIter,
        )
      }
    ...
    if (this.printDebug) {
      ...
      text(
        'x: ' +
          str(round(this.pos.x * 1000) / 1000) +
          '\ny: ' +
          str(round(this.pos.y * 1000) / 1000) +
          '\nzoom: ' +
          str(round((1 / this.zoom) * 1000) / 1000) +
          '\niterations: ' +
          str(round(this.maxIter * 1000) / 1000),
        ctlsBack + 10,
        15,
      )
    }
    ...
    if (0 < mouseX && mouseX < width && 0 < mouseY && mouseY < height) {
      text(
        'Mouse: (' +
          str(round(cX * 1000) / 1000.0) +
          ',' +
          str(round(cY * 1000) / 1000.0) +
          ')',
        ctlsBack + 10,
        height - 15,
      )
